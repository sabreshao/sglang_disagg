#!/bin/bash
# =============================================================================
# run_interactive_disagg_qwen3.sh - Interactive Entry Point for Qwen3-235B
# =============================================================================
# This script is the primary entry point for running a disaggregated inference
# job interactively (via salloc) with the Qwen3-235B model. It sets all
# environment variables and then delegates to run_xPyD_models.slurm, which
# launches Docker containers on each allocated node and starts the
# prefill/decode/router services inside them.
#
# Usage:
...
#                             value or a descending list like "512x256x128"
...
# -----------------------------------------------------------------------------
# Misc Configuration
# LOAD_DUMMY: Set to 1 to skip loading real model weights (use dummy weights).
#             Useful for quick smoke tests. Default: 0.
# DRY_RUN:    Set to 1 to print all commands without executing them.
#             Useful for validating configuration. Default: 0.
# Note: DOCKER_IMAGE_NAME defaults to the Qwen3-capable image defined in
#       run_xPyD_models.slurm. Override here if needed.
# -----------------------------------------------------------------------------
export LOAD_DUMMY=0
export DRY_RUN=0
