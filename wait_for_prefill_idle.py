#!/usr/bin/env python3
"""
Wait until all dp ranks on a prefill server have no running or waiting requests.

Usage:
    python wait_for_prefill_idle.py --prefill_url http://node04:8000
"""

import argparse
import json
import sys
import time
import urllib.error
import urllib.request

POLL_INTERVAL = 5       # seconds between each query
TIMEOUT = 20 * 60       # 20 minutes total


def fetch_loads(url: str) -> dict:
    req = urllib.request.Request(url, headers={"Accept": "application/json"})
    with urllib.request.urlopen(req, timeout=10) as resp:
        return json.loads(resp.read().decode())


def all_idle(data: dict) -> tuple[bool, list[dict]]:
    """Return (all_idle, busy_ranks)."""
    busy = [
        rank
        for rank in data.get("loads", [])
        if rank.get("num_running_reqs", 0) != 0
        or rank.get("num_waiting_reqs", 0) != 0
    ]
    return len(busy) == 0, busy


def main():
    parser = argparse.ArgumentParser(description="Wait until prefill server is idle.")
    parser.add_argument(
        "--prefill_url",
        dest="prefill_url",
        required=True,
        help="Base URL of the prefill server, e.g. http://node04:8000",
    )
    args = parser.parse_args()

    loads_url = args.prefill_url.rstrip("/") + "/v1/loads"
    deadline = time.monotonic() + TIMEOUT

    print(f"Polling {loads_url} every {POLL_INTERVAL}s (timeout {TIMEOUT//60}min) ...")

    while True:
        try:
            data = fetch_loads(loads_url)
            idle, busy_ranks = all_idle(data)

            if idle:
                dp_count = data.get("dp_rank_count", len(data.get("loads", [])))
                print(f"All {dp_count} dp ranks are idle. Done.")
                sys.exit(0)

            busy_summary = ", ".join(
                f"dp_rank={r['dp_rank']} running={r['num_running_reqs']} waiting={r['num_waiting_reqs']}"
                for r in sorted(busy_ranks, key=lambda x: x["dp_rank"])
            )
            print(f"[{time.strftime('%H:%M:%S')}] Still busy: {busy_summary}")

        except urllib.error.URLError as e:
            print(f"[{time.strftime('%H:%M:%S')}] Request failed: {e}")
        except (json.JSONDecodeError, KeyError) as e:
            print(f"[{time.strftime('%H:%M:%S')}] Bad response: {e}")

        if time.monotonic() >= deadline:
            print(f"Timeout after {TIMEOUT//60} minutes. Server still not idle.")
            sys.exit(1)

        time.sleep(POLL_INTERVAL)


if __name__ == "__main__":
    main()
