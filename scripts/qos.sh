...
for i in $(sudo nicctl show port | grep Port | awk '{print $3}'); do
    sudo nicctl update port -p "${i}" --pause-type pfc --rx-pause enable --tx-pause enable
done

for i in $(sudo nicctl show port | grep Port | awk '{print $3}'); do
    sudo nicctl update port --port "${i}" --auto-neg enable
done
